...
const paths = {
    ...
    assets: {
        dist: path.resolve(__dirname, 'build/assets/')
    },
    icons: path.resolve(__dirname, 'src/assets/img/svg-sprite/')
};

// Icon sprite setup
var iconsInSprite = [];
glob.sync(path.join(paths.icons, '**/*.svg')).forEach(function (file) {
...
export default {
    ...
    plugins: [
        svgBuilder({
            path: path.resolve(__dirname, 'src/assets/img/svg-sprite/'),
            prefix: 'icon',
            output: path.resolve(__dirname, 'src/assets/img/'),
            filename: 'svg-sprite',
            writeFile: true,
            position: 'end'
        }),
        handlebarsOverride({
            ...
            helpers: {
                ...
                assets: (options) => {
                    ...
                    return new handlebars.SafeString(path.relative(currentPath, paths.assets.dist).split(path.sep).join('/'));
                },
                countByKey: (data, key) => {
                    let count = 0;
    ...
    optimizeDeps: {
        include: [
            // Add the dependencies you need to pre-bundle here
            'vue',
            // other dependencies...
        ],
    },
